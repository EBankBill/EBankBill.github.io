define("app/marketingPlatform/detail", ["gallery/zepto/1.1.3/zepto", "mobile/utils/simple/simpleTips", "./_imgResize", "./_share", "./_fixedFix"],
function(a) {
    var b = a("gallery/zepto/1.1.3/zepto"),
    c = (a("mobile/utils/simple/simpleTips"), a("./_imgResize"));
    a("./_share");
    var d = a("./_fixedFix"),
    e = new d;
    e.fixBottom(".t_fix"),
    ...
    function() {
        var a = b(this);
        return a.prop("disabled") ? !1 : (a.prop("disabled", !0), void b.ajax({
            method: "post",
            url: pageConf.praiseUrl,
            data: pageConf.praiseData,
            success: function(c) {
                var d, e = JSON.parse(c);
                if (1 === e.code) {
                    var f = a.closest("ul").find(".longBtn");
                    f.length && (f.removeClass("none"), f.siblings().addClass("none")),
                    b(".vote_num").text( + b(".vote_num").text() + 1),
                    d = 0
                } else d = -2 === e.code ? 1 : -3 === e.code ? 2 : 3;
                var g = b(".vote_popup").eq(d);
                switch (d) {
                case 0:
                case 1:
                    e.msg ? g.find(".vote_msg").text(e.msg).show() : g.find(".vote_msg").hide(),
                    g.find(".vote_take").length > 0 && (e.take ? (g.find(".vote_take").hide(), g.find(".vote_index").show()) : (g.find(".vote_take").show(), g.find(".vote_index").hide()));
                    break;
                case 2:
                    break;
                default:
                    g.find("h4").text(e.msg)
                }
                g.removeClass("none")
            },
            error: function() {
                b(".vote_popup").find("h4").text("网络原因").removeClass("none")
            },
            complete: function() {
                a.prop("disabled", !1)
            }
    ...
    b("#voteAgin").on("click",
    function() {
        b("#voteBtn").trigger("click")
    }),
    b(".close", ".d-popup").on("click",
    function() {
        b(this).closest(".d-popup").addClass("none")
    }),
    b(".t_up>p").height() > b(".t_up").height() && b(".t_show").show().on("click",
    function() {
        b(".t_up").removeClass("t_up"),
        b(this).remove()
    }),
    b(".works_img").each(function() {
        var a = b(this),
        d = a.attr("src"),
        e = new c(a.get(0), d, 194, 194);
        e.init(),
        a.css("visibility", "visible")
    });
    var f = b("#headImg");
    if ("" != f.attr("src")) {
        var g = new c(f.get(0), f.attr("src"), 102, 102);
        g.init(),
        f.css("visibility", "visible")
    }
    b(".img_friends").length > 0 && b(".img_friends").each(function() {
        var a = b(this),
        d = a.attr("src");
        if ("" != d) {
            var e = new c(a.get(0), d, 64, 64);
            e.init(),
            a.css("visibility", "visible")
        }
    })
...
define("app/marketingPlatform/_fixedFix", ["gallery/zepto/1.1.3/zepto"],
function(a, b, c) {
    function d() {
        this.$top = e(),
        this.$bottom = e(),
    ...
    c.exports = d,
    d.prototype.fixTop = function(a) {
        this.$top = this.$top.add(a),
        this._calcNow(a)
    },
    d.prototype.fixBottom = function(a) {
        this.$bottom = this.$bottom.add(a),
        this._calcNow(a)
    },
    d.prototype._calcNow = function(a) {
        e(a).css("position", "absolute"),
        e(window).trigger("scroll")
    },
    d.prototype._bind = function() {
        var a = this;
        e(window).on("scroll touchmove",
        function() {
            e(window).height() + e(window).scrollTop() <= e(document).height() && e(window).scrollTop() >= 0 && (a.$bottom.each(function() {
                e(this).css("top", e(window).height() - e(this).height() + e(window).scrollTop())
            }), a.$top.css("top", e(window).scrollTop()))
        })
    }
});